...
Now it's time to monitor the service. We shall use for this purpose the Grafana and Prometheus deployments from the Open Data Hub's instance (the `opendatahub` project).
Since the resources we want to monitor are in a different namespace, first we must setup the right ACL for the `opendatahub` namespace to access resources from the `xraylab` namespace. To achieve that we must add a role binding to the `opendatahub` project.
In the Administrator view open User Management->Role Bindings and click on Create binding
![odh-role-binding-1](docs/odh-role-binding-1-new.png)
...
![odh-role-binding-2](docs/odh-role-binding-2-new.png)
...
![odh-prom-1](docs/odh-prom-1-new.png)
...
![odh-prom-2](docs/odh-prom-2-new.png)
